...
async function handleAfterAgentHookImpl(args) {
    ...
    if (runState === "waiting" && onlyBreakpointsPending) {
        ...
        if (verbose) {
            process.stderr.write(`[hook:run after-agent] Run waiting on breakpoint(s) — allowing exit for human resolution\n`);
        }
        if (runId) {
            await appendStopHookEvent(path.join(runsDir, runId), {
                sessionId,
...
async function installGeminiHarness(options) {
...
async function installGeminiPlugin(options) {
    const targetDir = (0, installSupport_1.getGeminiExtensionDir)(options.workspace);
    if ((0, installSupport_1.isGeminiPluginInstalled)(options.workspace)) {
        return {
            harness: HARNESS_NAME,
