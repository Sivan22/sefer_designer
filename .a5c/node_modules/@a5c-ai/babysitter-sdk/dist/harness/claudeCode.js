 ...
 */
function isOnlyBreakpoints(pendingByKind) {
...
async function handleStopHookImpl(args) {
    ...
    let sessionId = safeStr(hookInput, "session_id");
    if (!sessionId) {
    ...
    if (!lastText) {
        ...
        if (hookLastMsg) {
            ...
            promiseValue = (0, session_2.extractPromiseTag)(hookLastMsg);
            hasPromise = promiseValue !== null;
    ...
    if (runId) {
        try {
            ...
            if (!(0, node_fs_1.existsSync)(path.join(runDir, "run.json")) && !path.isAbsolute(runId)) {
                ...
                for (const alt of alternatives) {
                    ...
                    if (resolved !== path.resolve(runDir) && (0, node_fs_1.existsSync)(path.join(resolved, "run.json"))) {
        ...
        if (!runState) {
            ...
            if (verbose) {
                process.stderr.write(`[hook:run stop] Run state is empty for ${runId}; run may be misconfigured — preserving session file\n`);
            }
            if (runId) {
    ...
    if (runState === "waiting" && onlyBreakpointsPending) {
        ...
        if (verbose) {
            process.stderr.write(`[hook:run stop] Run waiting on breakpoint(s) — allowing exit for human resolution\n`);
        }
        if (runId) {
            await appendStopHookEvent(path.join(runsDir, runId), {
                sessionId,
    ...
    let compressionCfg = null;
    try {
        compressionCfg = (0, config_loader_1.loadCompressionConfig)(process.cwd());
    }
    catch {
        // Best-effort
    }
    ...
    if (resolvedPluginRoot) {
        try {
            ...
            const relevantSkills = (discoverResult.skills || []).filter((s) => !EXCLUDED_SKILLS.has(s.name.toLowerCase()));
            ...
            if (compressionCfg?.enabled && cacheLayer?.enabled) {
                ...
                const sections = [];
                const libraryItems = [
                    ...relevantSkills.slice(0, 4).map((s) => ({ kind: "Skill", name: s.name, file: s.file })),
                    ...relevantAgents.slice(0, 2).map((a) => ({ kind: "Agent", name: a.name, file: a.file })),
                ];
                for (const item of libraryItems) {
                    if (!item.file)
                        continue;
                    const content = (0, library_cache_1.getOrCompressFile)(item.file, cacheLayer.targetReduction, cacheLayer.ttlHours, cacheDir);
                    if (content) {
    ...
    if (compressionCfg?.enabled && compressionCfg.layers.sdkContextHook.enabled) {
        ...
        if ((0, density_filter_1.estimateTokens)(prompt) > sdkLayer.minCompressionTokens) {
            effectivePrompt = (0, density_filter_1.densityFilterText)(prompt, sdkLayer.targetReduction);
        }
    }
...
async function handleSessionStartHookImpl(args) {
    ...
    if (envFile) {
        ...
        process.stderr.write(`[hook:run session-start] CLAUDE_ENV_FILE not set — session ID will not be persisted to env file\n`);
    }
    ...
    if (resolvedPluginRoot) {
        try {
            ...
            if (compressionCfg.enabled && cacheLayer.enabled) {
                const cacheDir = path.join(process.cwd(), ".a5c", "cache", "compression");
                const libraryFiles = (0, library_cache_1.findLibraryFiles)(resolvedPluginRoot);
    ...
    if (!envFilePersisted && !stateFilePersisted) {
        process.stderr.write(`[hook:run session-start] Session persistence failed — neither env file nor state file was written\n`);
        process.stdout.write("{}\n");
        return 1;
    }
...
async function installClaudeCodePlugin(options) {
    ...
    const marketplaceResult = await (0, installSupport_1.execFilePromise)("claude", [
        "plugin",
        "marketplace",
        "add",
        "a5c-ai/babysitter",
...
function createClaudeCodeAdapter() {
    return {
        ...
        getPromptContext(opts) {
            return (0, context_1.createClaudeCodeContext)(opts);
        },
    };
}
