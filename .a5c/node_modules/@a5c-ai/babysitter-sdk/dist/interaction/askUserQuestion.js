...
exports.validateAskUserQuestionRequest = validateAskUserQuestionRequest;
exports.createAskUserQuestionResponse = createAskUserQuestionResponse;
exports.createDefaultAskUserQuestionResponse = createDefaultAskUserQuestionResponse;
exports.createApprovalAskUserQuestion = createApprovalAskUserQuestion;
exports.createReadlineAskUserQuestionUiContext = createReadlineAskUserQuestionUiContext;
exports.promptAskUserQuestionWithUiContext = promptAskUserQuestionWithUiContext;
exports.promptAskUserQuestionWithReadline = promptAskUserQuestionWithReadline;
const RESET = "\x1b[0m";
const BOLD = "\x1b[1m";
const CYAN = "\x1b[36m";
const DIM = "\x1b[2m";
...
function isTTYInput(stream) {
    return "isTTY" in stream && stream.isTTY === true && typeof stream.setRawMode === "function";
}
/**
 * Render the option list to `output`. Returns the number of lines written.
 */
function renderOptions(output, options, cursor, selected, multiSelect) {
    let lines = 0;
 ...
 */
function promptArrowKeySelect(input, output, options, opts) {
...
function parseOptionAnswer(rawAnswer, question) {
...
function validateAskUserQuestionRequest(request) {
    if (!Array.isArray(request.questions)) {
        throw new Error("AskUserQuestion requires a questions array.");
    }
    if (request.questions.length < MIN_QUESTION_COUNT ||
        request.questions.length > MAX_QUESTION_COUNT) {
        throw new Error(`AskUserQuestion requires ${MIN_QUESTION_COUNT}-${MAX_QUESTION_COUNT} questions.`);
    }
    for (const question of request.questions) {
        ...
        if (question.options) {
            if (question.options.length < MIN_OPTION_COUNT ||
                ...
                throw new Error(`AskUserQuestion option-based questions require ${MIN_OPTION_COUNT}-${MAX_OPTION_COUNT} options.`);
            }
...
function createAskUserQuestionResponse(request, answers) {
...
function createDefaultAskUserQuestionResponse(request) {
    return createAskUserQuestionResponse(request, buildDefaultAnswers(request));
}
function createApprovalAskUserQuestion(question, header = "Decision") {
    return {
        questions: [
            {
                header,
...
function formatUiPromptTitle(question) {
...
function buildUiTitle(question) {
    const header = question.header?.trim();
    const body = formatUiPromptTitle(question);
    return header ? `${header}\n\n${body}` : body;
}
function createReadlineAskUserQuestionUiContext(rl) {
    return {
        async select(title, options) {
            process.stderr.write(`${title}\n`);
            // Use arrow-key selector when stdin is a TTY
            if (isTTYInput(process.stdin)) {
                const idx = await promptArrowKeySelect(process.stdin, process.stderr, options);
...
async function promptAskUserQuestionWithUiContext(ui, request) {
    return runAskUserQuestionPrompt(request, async () => {
        ...
        for (const [index, question] of request.questions.entries()) {
            ...
            if (options.length === 0) {
                let resolved = false;
                while (!resolved) {
                    const answer = (await ui.input(buildUiTitle(question), question.required ? "Answer (required)" : "Answer"))?.trim();
                    if (answer) {
                        answers[key] = answer;
                        resolved = true;
                        continue;
            ...
            if (question.multiSelect) {
                let resolved = false;
                while (!resolved) {
                    const rawAnswer = (await ui.input(buildUiTitle(question), "Enter one or more options (numbers or labels, comma-separated)"))?.trim() ?? "";
            ...
            let resolved = false;
            while (!resolved) {
                const selection = await ui.select(buildUiTitle(question), selectOptions);
                if (!selection) {
                    if (question.required) {
                        continue;
                    }
                ...
                if (allowOther && selection === "Other (type a custom answer)") {
                    const other = (await ui.input(buildUiTitle(question), "Type your answer"))?.trim();
                    if (other) {
                        answers[key] = other;
                        resolved = true;
                        continue;
...
function getDefaultAnswerForQuestion(question) {
    const options = question.options ?? [];
...
function buildDefaultAnswers(request) {
    const answers = {};
    for (const [index, question] of request.questions.entries()) {
        const key = getQuestionKey(question, index);
...
function runAskUserQuestionPrompt(request, runInteractive) {
    validateAskUserQuestionRequest(request);
    ...
    return new Promise((resolve) => {
        ...
        const timer = setTimeout(() => {
            ...
            settled = true;
            process.stderr.write(`${YELLOW}Ask timeout reached (${request.timeout}ms). Auto-selecting defaults.${RESET}\n`);
...
async function promptAskUserQuestionWithReadline(rl, request) {
    return runAskUserQuestionPrompt(request, () => promptAskUserQuestionInteractive(rl, request));
}
async function promptAskUserQuestionInteractive(rl, request) {
    const answers = {};
    for (const [index, question] of request.questions.entries()) {
        const key = getQuestionKey(question, index);
        const options = question.options ?? [];
        ...
        if (options.length > 0 && isTTYInput(process.stdin)) {
            ...
            if (question.multiSelect) {
                const result = await promptArrowKeySelect(process.stdin, process.stderr, optionLabels, { multiSelect: true });
                ...
                const result = await promptArrowKeySelect(process.stdin, process.stderr, optionLabels);
