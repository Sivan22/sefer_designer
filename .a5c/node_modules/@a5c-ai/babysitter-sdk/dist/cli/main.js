...
exports._collapseDoubledA5cRuns = void 0;
...
const compressionSet_1 = require("./commands/compressionSet");
const compressOutput_1 = require("./commands/compressOutput");
const harnessInstall_1 = require("./commands/harnessInstall");
...
function parseArgs(argv) {
    ...
    else if (parsed.command === "harness:retrospect" ||
...
const collapseDoubledA5cRuns = resolveInputPath_1.collapseDoubledA5cRuns;
...
function _logPendingActions(actions, options = {}) {
...
function _enrichIterationMetadata(metadata, pendingActions) {
...
function _logSleepHints(command, actions) {
    for (const action of actions) {
...
async function readInputsFile(filePath) {
    const absolute = (0, resolveInputPath_1.resolveInputPath)(filePath);
    let contents;
    try {
...
async function handleRunRepairJournal(parsed) {
    ...
    for (const filename of files) {
        ...
        let payload;
        try {
            payload = JSON.parse(await node_fs_1.promises.readFile(fullPath, "utf8"));
        }
        catch {
            droppedCorrupt++;
            continue;
...
async function handleTaskPost(parsed) {
    ...
    const resolveMaybeRunRelative = (candidate) => {
        ...
        return (0, resolveInputPath_1.collapseDoubledA5cRuns)(path.join(runDir, candidate));
    };
...
// Harness commands
// ---------------------------------------------------------------------------
function handleHarnessHelp(parsed) {
...
async function handleHarnessObserve(parsed) {
    ...
    const child = spawn("npx", ["-y", "@a5c-ai/babysitter-observer-dashboard@latest", "--watch-dir", watchDir], { stdio: "inherit", shell: true });
...
async function handleHarnessDiscover(parsed) {
    const { discoverHarnesses, detectCallerHarness } = await Promise.resolve().then(() => __importStar(require("../harness/discovery")));
    ...
    if (parsed.json) {
        ...
        for (const r of results) {
            ...
            console.log(`  ${r.name.padEnd(14)}  ${(r.installed ? "yes" : "no").padEnd(9)}  ${version}  ${(r.configFound ? "yes" : "no").padEnd(7)}  ${caps}`);
        }
        if (caller) {
...
async function handleHarnessInvoke(parsed) {
    const { invokeHarness } = await Promise.resolve().then(() => __importStar(require("../harness/invoker")));
    const harnessName = parsed.positional?.[0];
    if (!harnessName) {
        const error = new exceptions_1.BabysitterRuntimeError("MissingArgument", "harness:invoke requires a harness name as the first argument", {
            category: exceptions_1.ErrorCategory.Validation,
            suggestions: ["babysitter harness:invoke claude-code --prompt \"hello\""],
        });
        if (parsed.json) {
            console.error(JSON.stringify((0, exceptions_1.toStructuredError)(error), null, 2));
        }
        else {
            console.error((0, exceptions_1.formatErrorWithContext)(error, { colors: supportsColors() }));
        }
        return 1;
    }
    const prompt = parsed.prompt;
    if (!prompt) {
        const error = new exceptions_1.BabysitterRuntimeError("MissingArgument", "harness:invoke requires --prompt <text>", {
            category: exceptions_1.ErrorCategory.Validation,
    ...
    try {
        ...
        if (parsed.json) {
            const structured = err instanceof exceptions_1.BabysitterRuntimeError
                ? (0, exceptions_1.toStructuredError)(err)
                : { error: err instanceof Error ? err.message : String(err) };
            console.error(JSON.stringify(structured, null, 2));
        }
        else {
            if (err instanceof exceptions_1.BabysitterRuntimeError) {
        ...
        async run(argv = process.argv.slice(2)) {
            ...
            try {
                ...
                if (parsed.command === "log") {
                    return await (0, log_1.handleLog)({
                ...
                if (parsed.command === "hook:run") {
                    ...
                    const { detectCallerHarness } = await Promise.resolve().then(() => __importStar(require("../harness/discovery")));
                ...
                if (parsed.command?.startsWith("instructions:")) {
                    ...
                    return await (0, instructions_1.handleInstructionsCommand)(instructionsArgs);
                }
                // Profile commands
                if (parsed.command === "profile:read" ||
                    ...
                    return await (0, profile_1.handleProfileCommand)(subcommand, profileArgs);
                }
                // Plugin commands
                if (parsed.command?.startsWith("plugin:")) {
                    ...
                    if (parsed.command === "plugin:add-marketplace") {
                        return await (0, plugin_1.handlePluginAddMarketplace)(pluginArgs);
                    }
                    if (parsed.command === "plugin:update-marketplace") {
                        return await (0, plugin_1.handlePluginUpdateMarketplace)(pluginArgs);
                    }
                    if (parsed.command === "plugin:list-plugins") {
                        return await (0, plugin_1.handlePluginListPlugins)(pluginArgs);
                    }
                    if (parsed.command === "plugin:install") {
                        return await (0, plugin_1.handlePluginInstall)(pluginArgs);
                    }
                    if (parsed.command === "plugin:uninstall") {
                        return await (0, plugin_1.handlePluginUninstall)(pluginArgs);
                    }
                    if (parsed.command === "plugin:update") {
                        return await (0, plugin_1.handlePluginUpdate)(pluginArgs);
                    }
                    if (parsed.command === "plugin:configure") {
                        return await (0, plugin_1.handlePluginConfigure)(pluginArgs);
                    }
                    if (parsed.command === "plugin:list-installed") {
                        return await (0, plugin_1.handlePluginListInstalled)(pluginArgs);
                    }
                    if (parsed.command === "plugin:update-registry") {
                        return await (0, plugin_1.handlePluginUpdateRegistry)(pluginArgs);
                    }
                    if (parsed.command === "plugin:remove-from-registry") {
                        return await (0, plugin_1.handlePluginRemoveFromRegistry)(pluginArgs);
                    }
                }
                // Harness commands
                ...
                if (parsed.command === "harness:install") {
                    return await (0, harnessInstall_1.handleHarnessInstall)({
                ...
                if (parsed.command === "harness:install-plugin") {
                    return await (0, harnessInstall_1.handleHarnessInstallPlugin)({
                        harnessName: parsed.positional?.[0],
                        workspace: parsed.workspace,
                        json: parsed.json,
                        dryRun: parsed.dryRun,
                ...
                if (parsed.command === "harness:forever") {
                    const { handleHarnessCreateRun } = await Promise.resolve().then(() => __importStar(require("./commands/harnessCreateRun")));
                    const foreverPrompt = (0, prompts_1.renderCommandTemplate)("forever", {
                ...
                if (parsed.command === "harness:resume-run" || parsed.command === "harness:resume") {
                    const { handleHarnessResumeRun } = await Promise.resolve().then(() => __importStar(require("./commands/harnessResumeRun")));
                ...
                if (parsed.command === "harness:retrospect") {
                    ...
                    const retrospectPrompt = (0, prompts_1.renderCommandTemplate)("retrospect", {
                        targetRunText,
                        additionalInstructions: parsed.prompt ?? "",
                    });
                    return await handleHarnessCreateRun({
                ...
                if (parsed.command === "harness:cleanup") {
                    ...
                    const cleanupPrompt = (0, prompts_1.renderCommandTemplate)("cleanup", {
                ...
                if (parsed.command === "harness:assimilate") {
                    const { handleHarnessCreateRun } = await Promise.resolve().then(() => __importStar(require("./commands/harnessCreateRun")));
                    const assimilatePrompt = (0, prompts_1.renderCommandTemplate)("assimilate", {
                ...
                if (parsed.command === "harness:doctor") {
                    const { handleHarnessCreateRun } = await Promise.resolve().then(() => __importStar(require("./commands/harnessCreateRun")));
                    const doctorPrompt = (0, prompts_1.renderCommandTemplate)("doctor", {
                ...
                if (parsed.command === "harness:contrib") {
                    const { handleHarnessCreateRun } = await Promise.resolve().then(() => __importStar(require("./commands/harnessCreateRun")));
                    const contribPrompt = (0, prompts_1.renderCommandTemplate)("contrib", {
                ...
                if (parsed.command === "harness:user-install") {
                    const { handleHarnessCreateRun } = await Promise.resolve().then(() => __importStar(require("./commands/harnessCreateRun")));
                    const userInstallPrompt = (0, prompts_1.renderCommandTemplate)("user-install", {
                        additionalInstructions: parsed.prompt ?? "",
                    });
                    return await handleHarnessCreateRun({
                ...
                if (parsed.command === "harness:project-install") {
                    const { handleHarnessCreateRun } = await Promise.resolve().then(() => __importStar(require("./commands/harnessCreateRun")));
                    const projectInstallPrompt = (0, prompts_1.renderCommandTemplate)("project-install", {
                        additionalInstructions: parsed.prompt ?? "",
                    });
                    return await handleHarnessCreateRun({
                ...
                if (parsed.command === "mcp:serve") {
                    return await (0, mcpServe_1.handleMcpServe)({ json: parsed.json });
                }
                ...
                if (parsed.command === "tokens:stats") {
                    return await (0, tokensStats_1.handleTokensStats)({
                ...
                if (parsed.command === "compression:toggle") {
                    ...
                    return await (0, compressionToggle_1.handleCompressionToggle)({
                ...
                if (parsed.command === "compression:set") {
                    ...
                    return await (0, compressionSet_1.handleCompressionSet)({
                ...
                if (parsed.command === "compression:reset") {
                    return await (0, compressionReset_1.handleCompressionReset)({ json: parsed.json });
                }
                if (parsed.command === "compress-output") {
                    return (0, compressOutput_1.handleCompressOutput)({ args: parsed.compressOutputArgs ?? [] });
                }
                // This should not be reached due to the VALID_COMMANDS check above
                return handleUnknownCommand(parsed.command, parsed.json);
            }
            catch (error) {
                if (parsedJson && error instanceof exceptions_1.BabysitterRuntimeError && error.name === "UnsupportedHarnessInstall") {
                    return (0, harnessInstall_1.formatHarnessInstallError)(error, true);
                }
