...
exports.runProcessDefinitionPhase = runProcessDefinitionPhase;
...
async function waitForProcessFile(filePath, timeoutMs = POLL_TIMEOUT_MS) {
    ...
    throw new harnessUtils_1.BabysitterRuntimeError("ProcessFileTimeoutError", `Process file was not created within ${timeoutMs / 1_000}s: ${filePath}`, {
        category: harnessUtils_1.ErrorCategory.External,
        nextSteps: [
...
function sanitizeJavaScriptForStructuralChecks(source) {
...
function hasNamedProcessGlobalReferenceConflict(source) {
...
function assumesRuntimeWorkspacePathWithoutModuleFallback(source) {
    const normalized = sanitizeJavaScriptForStructuralChecks(source).replace(/\r\n/g, "\n");
    const usesContextWorkspacePath = /\bctx\??\.workspaceDir\b/.test(normalized) ||
...
async function validateProcessExport(filePath) {
    ...
    if (syntaxCheck.exitCode !== 0) {
        const diagnostic = [syntaxCheck.stdout, syntaxCheck.stderr]
            ...
            .filter(Boolean)
            .join("\n");
        throw new harnessUtils_1.BabysitterRuntimeError("InvalidProcessSyntaxError", diagnostic
    ...
    if (hasNamedProcessGlobalReferenceConflict(source)) {
        throw new harnessUtils_1.BabysitterRuntimeError("InvalidProcessSourceError", `Process file at ${filePath} references \`process.\` inside the named 'process' export, which shadows Node's global process object`, {
            category: harnessUtils_1.ErrorCategory.Validation,
            nextSteps: [
    ...
    if (assumesRuntimeWorkspacePathWithoutModuleFallback(source)) {
        throw new harnessUtils_1.BabysitterRuntimeError("InvalidProcessSourceError", `Process file at ${filePath} assumes ctx.workspaceDir or ctx.cwd exists, but the runtime process context does not provide workspace paths`, {
            category: harnessUtils_1.ErrorCategory.Validation,
            nextSteps: [
    ...
    if (unresolvedPlaceholders.length > 0) {
        throw new harnessUtils_1.BabysitterRuntimeError("InvalidProcessSourceError", `Process file at ${filePath} contains unresolved template placeholders: ${unresolvedPlaceholders.join(", ")}`, {
            category: harnessUtils_1.ErrorCategory.Validation,
            nextSteps: [
    ...
    const moduleUrl = `${(0, node_url_1.pathToFileURL)(path.resolve(filePath)).href}?t=${Date.now()}-${++processValidationImportNonce}`;
    (0, registry_1.resetGlobalTaskRegistry)();
    let mod;
    try {
    ...
    if (typeof fn !== "function") {
        throw new harnessUtils_1.BabysitterRuntimeError("InvalidProcessExportError", `Process file at ${filePath} does not export a function named 'process'`, {
            category: harnessUtils_1.ErrorCategory.Validation,
            nextSteps: [
    ...
    if (defineTaskBlocks.length === 0) {
        throw new harnessUtils_1.BabysitterRuntimeError("InvalidProcessSourceError", `Process file at ${filePath} does not define any babysitter tasks via defineTask(...)`, {
            category: harnessUtils_1.ErrorCategory.Validation,
            nextSteps: [
    ...
    if (!hasCtxTaskInvocation(source)) {
        throw new harnessUtils_1.BabysitterRuntimeError("InvalidProcessSourceError", `Process file at ${filePath} does not invoke any babysitter tasks through ctx.task(...)`, {
            category: harnessUtils_1.ErrorCategory.Validation,
            nextSteps: [
    ...
    if (taskIdsMissingKind.length > 0) {
        throw new harnessUtils_1.BabysitterRuntimeError("InvalidProcessSourceError", `Process file at ${filePath} defines task(s) without a top-level kind: ${taskIdsMissingKind.join(", ")}`, {
            category: harnessUtils_1.ErrorCategory.Validation,
            nextSteps: [
    ...
    if (taskKindShapeMismatches.length > 0) {
        throw new harnessUtils_1.BabysitterRuntimeError("InvalidProcessSourceError", `Process file at ${filePath} has task definition kind mismatches: ${taskKindShapeMismatches
    ...
    if (agentTaskIds.length === 0) {
        throw new harnessUtils_1.BabysitterRuntimeError("InvalidProcessSourceError", `Process file at ${filePath} does not define any agent tasks`, {
            category: harnessUtils_1.ErrorCategory.Validation,
            nextSteps: [
...
async function resolvePhase1ProcessLibraryRoot(scope = "binding") {
    const active = await (0, active_1.ensureActiveProcessLibrary)();
    const bindingRoot = active.binding?.dir;
    if (!bindingRoot) {
        throw new harnessUtils_1.BabysitterRuntimeError("ProcessLibraryNotResolved", "No active process library binding is available.", { category: harnessUtils_1.ErrorCategory.Runtime });
    }
    const root = scope === "clone"
...
function parseRipgrepMatchLine(line, root) {
...
async function searchProcessLibrary(query, scope = "binding", limit = harnessUtils_1.PROCESS_LIBRARY_SEARCH_DEFAULT_LIMIT) {
    const trimmedQuery = query.trim();
    if (!trimmedQuery) {
        throw new harnessUtils_1.BabysitterRuntimeError("ProcessLibrarySearchQueryMissing", "Process-library search query must not be empty.", { category: harnessUtils_1.ErrorCategory.Validation });
    }
    const normalizedLimit = Math.max(1, Math.min(limit, 40));
    const { root } = await resolvePhase1ProcessLibraryRoot(scope);
    const matches = [];
    const seen = new Set();
    const contentResult = await execShellEffect("rg", [
        "-n",
        "--no-heading",
        "--color",
        "never",
...
async function readProcessLibraryFile(relativePath, scope = "binding") {
    const trimmedPath = relativePath.trim();
    if (!trimmedPath) {
        throw new harnessUtils_1.BabysitterRuntimeError("ProcessLibraryReadPathMissing", "Process-library file path must not be empty.", { category: harnessUtils_1.ErrorCategory.Validation });
    }
    const { root } = await resolvePhase1ProcessLibraryRoot(scope);
    const absolutePath = path.resolve(root, trimmedPath);
    if (!isPathWithinRoot(root, absolutePath)) {
        throw new harnessUtils_1.BabysitterRuntimeError("ProcessLibraryReadPathOutsideRoot", `Process-library file must stay under ${root}.`, { category: harnessUtils_1.ErrorCategory.Validation });
    }
    const content = await node_fs_1.promises.readFile(absolutePath, "utf8");
    return {
        scope,
        root,
        relativePath: trimmedPath,
        absolutePath,
        content: content.slice(0, harnessUtils_1.PROCESS_LIBRARY_READ_MAX_CHARS),
        truncated: content.length > harnessUtils_1.PROCESS_LIBRARY_READ_MAX_CHARS,
    };
}
...
function coerceAgentResultValue(taskDef, output) {
    ...
    if (!candidate) {
        throw new harnessUtils_1.BabysitterRuntimeError("AgentOutputSchemaMismatch", "Agent task declared outputSchema but did not return JSON output", { category: harnessUtils_1.ErrorCategory.External });
    }
    try {
        return JSON.parse(candidate);
    }
    catch (error) {
        throw new harnessUtils_1.BabysitterRuntimeError("AgentOutputSchemaMismatch", error instanceof Error
            ? `Agent task declared outputSchema but returned invalid JSON: ${error.message}`
            : "Agent task declared outputSchema but returned invalid JSON", { category: harnessUtils_1.ErrorCategory.External });
    }
}
// ── Process Recovery ─────────────────────────────────────────────────
...
function normalizeProcessDefinitionSource(source) {
...
async function recoverReportedProcessDefinition(args) {
    ...
    if (recovered) {
        ...
        (0, harnessUtils_1.writeVerboseBlock)(args.verbose, args.json, "phase1 recovered report", recovered);
    }
    return recovered ?? undefined;
}
function writeVerboseProcessDefinitionRecovery(json) {
    if (!json) {
        process.stderr.write(`${harnessUtils_1.DIM}Phase 1 recovery: the agent did not report the process file, retrying with an explicit write-and-report instruction...${harnessUtils_1.RESET}\n`);
    }
}
...
async function _runExternalProcessDefinitionPhase(args) {
    ...
    const invokeProcessAuthor = async (label, prompt, timeout) => {
        ...
        if (!result.success) {
            ...
            throw new harnessUtils_1.BabysitterRuntimeError("ProcessDefinitionFailed", result.output, { category: harnessUtils_1.ErrorCategory.External });
        }
    ...
    if (!report?.processPath) {
        ...
        throw new harnessUtils_1.BabysitterRuntimeError("ProcessDefinitionReportMissing", "The process-definition harness did not produce a valid process file or recoverable JavaScript output.", { category: harnessUtils_1.ErrorCategory.Runtime });
    }
    await invokeProcessAuthor("phase1 sdk conformance", buildExternalProcessConformancePrompt({
    ...
    if (!args.json) {
        process.stderr.write(`${harnessUtils_1.GREEN}Phase 1 complete:${harnessUtils_1.RESET} process=${harnessUtils_1.CYAN}${report.processPath}${harnessUtils_1.RESET}\n`);
    }
...
async function runProcessDefinitionPhase(args) {
    ...
    const customTools = [
        ...
        },
        {
            ...
            description: "Resolve the active shared process-library binding, bootstrapping it if needed, and return the active roots that phase 1 must inspect.",
            parameters: typebox_1.Type.Object({}),
            execute: async () => {
                const active = await (0, active_1.ensureActiveProcessLibrary)();
                writeVerboseData("phase1 tool babysitter_resolve_process_library", active, 1200);
                return (0, harnessUtils_1.formatToolResult)(active, "Active shared process library resolved.");
            },
        },
        {
            name: "babysitter_search_process_library",
            label: "Search Process Library",
            description: "Search the active shared process library by content and path before authoring a process.",
            parameters: typebox_1.Type.Object({
                query: typebox_1.Type.String(),
                scope: typebox_1.Type.Optional(typebox_1.Type.Union([
                    typebox_1.Type.Literal("binding"),
                    typebox_1.Type.Literal("clone"),
                    typebox_1.Type.Literal("reference"),
                ])),
                limit: typebox_1.Type.Optional(typebox_1.Type.Number({ minimum: 1, maximum: 40 })),
            }),
            execute: async (_toolCallId, params) => {
                writeVerboseData("phase1 tool babysitter_search_process_library request", params);
                const result = await searchProcessLibrary(params.query, params.scope ?? "binding", params.limit);
                writeVerboseData("phase1 tool babysitter_search_process_library result", result, 2000);
                return (0, harnessUtils_1.formatToolResult)(result, "Process-library search completed.");
            },
        },
        {
            name: "babysitter_read_process_library_file",
            label: "Read Process Library File",
            description: "Read a specific file from the active shared process library after you identify it via search.",
            parameters: typebox_1.Type.Object({
                relativePath: typebox_1.Type.String(),
                scope: typebox_1.Type.Optional(typebox_1.Type.Union([
                    typebox_1.Type.Literal("binding"),
                    typebox_1.Type.Literal("clone"),
                    typebox_1.Type.Literal("reference"),
            ...
            execute: async (_toolCallId, params) => {
                ...
                const result = await readProcessLibraryFile(params.relativePath, params.scope ?? "binding");
                writeVerboseData("phase1 tool babysitter_read_process_library_file result", {
                    ...result,
                    contentPreview: (0, harnessUtils_1.truncateForVerboseLog)(result.content, 1200),
                }, 2000);
                return (0, harnessUtils_1.formatToolResult)(result, "Process-library file read completed.");
            },
        },
        {
        ...
        },
        {
            ...
            parameters: typebox_1.Type.Object({
                processPath: typebox_1.Type.String(),
            ...
            execute: (_toolCallId, params) => {
                ...
                return (0, harnessUtils_1.formatToolResult)(state.report, "Process definition reported.");
            },
        },
    ];
    const agenticTools = (0, agenticTools_1.createAgenticToolDefinitions)({
    ...
    (0, harnessUtils_1.emitProgress)({ phase: "1", status: "started", harness: "internal (agentic)" }, args.json, args.verbose);
    ...
    const phase1ToolsMode = workspaceAssessment.kind === "empty"
        ? "default"
        : "coding";
    session = (0, harnessUtils_1.createPiSession)({
        workspace: args.workspace,
        model: args.model,
        thinkingLevel: "low",
    ...
    try {
        ...
        if (!args.json) {
            process.stderr.write(`${harnessUtils_1.DIM}Phase 1 agent is defining the process...${harnessUtils_1.RESET}\n`);
        ...
        if (!result.success) {
            ...
            writeVerbose("[phase1 recovery] proceeding with the reported process file after a late PI prompt failure");
        }
        else {
        ...
        if (!state.report?.processPath) {
            ...
            const recovery = await (0, harnessUtils_1.promptPiWithRetry)({
                session,
            ...
            if (!recovery.success) {
                ...
                if (!recovered?.processPath) {
                    throw new harnessUtils_1.BabysitterRuntimeError("ProcessDefinitionFailed", recovery.output, { category: harnessUtils_1.ErrorCategory.External });
                }
                writeVerbose("[phase1 recovery] using the reported process file after the recovery prompt failed late");
            }
            else {
        ...
        if (!state.report?.processPath) {
            ...
            const finalRecovery = await (0, harnessUtils_1.promptPiWithRetry)({
                session,
            ...
            if (!finalRecovery.success) {
                ...
                if (!recovered?.processPath) {
                    throw new harnessUtils_1.BabysitterRuntimeError("ProcessDefinitionFailed", finalRecovery.output, { category: harnessUtils_1.ErrorCategory.External });
                }
                writeVerbose("[phase1 recovery] using the reported process file after the final recovery prompt failed late");
            }
            else {
        ...
        if (!state.report?.processPath) {
            writeVerboseData("phase1 unrecoverable outputs", phaseOutputs);
            if (!args.interactive) {
                throw new harnessUtils_1.BabysitterRuntimeError("ProcessDefinitionReportMissing", "The process-definition agent finished without calling babysitter_report_process_definition, and no recoverable process file or code output was produced.", { category: harnessUtils_1.ErrorCategory.Runtime });
            }
            else {
                throw new harnessUtils_1.BabysitterRuntimeError("ProcessDefinitionReportMissing", "The process-definition agent finished without calling babysitter_report_process_definition, and no recoverable process file or code output was produced.", { category: harnessUtils_1.ErrorCategory.Runtime });
            }
        ...
        for (let repairAttempt = 0; repairAttempt < 3; repairAttempt += 1) {
            try {
                ...
                const repair = await (0, harnessUtils_1.promptPiWithRetry)({
                    session,
        ...
        writeVerboseData("phase1 error", error instanceof Error
            ? {
                name: error.name,
