...
exports.parseProcessFileMarkers = parseProcessFileMarkers;
exports.discoverFromProcessFile = discoverFromProcessFile;
...
async function findProcessFiles(dir) {
    try {
        const resolvedDir = path.resolve(dir);
        const entries = await node_fs_1.promises.readdir(resolvedDir, { withFileTypes: true });
        return entries
 ...
 */
function capPerSpecialization(items, cap) {
 ...
 */
function parseProcessFileMarkers(filePath) {
    let content;
    try {
 ...
 * fallback.
 */
function resolveMarkersToMetadata(markers, options) {
 ...
 */
function discoverFromProcessFile(options) {
...
function resolveLegacyPluginProcessRoot(pluginRoot) {
    return path.join(pluginRoot, 'skills', 'babysit', 'process');
}
function findRepoLibraryRoot(start) {
...
async function resolveDiscoveryProcessRoot(options) {
    try {
        const active = await (0, active_1.resolveActiveProcessLibrary)({
...
async function discoverSkillsInternal(options) {
    ...
    const filteredPluginSkills = pluginSkills.filter(s => !(s.file?.replace(/\\/g, '/').includes('/specializations/')));
