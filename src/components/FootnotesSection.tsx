...
export function FootnotesSection({
  ...
  if (layout === 'single-torah' || layout === 'single-story') {
    ...
    const size = isTorah ? 'text-[13.5pt] leading-snug' : 'text-[7pt] leading-relaxed'
    return (
      ...
      <div className="border-t border-black/15 pt-3">
        <div className="grid grid-cols-2 gap-6">
          ...
          <div>
            ...
            <div className="space-y-2">
              {torahFootnotes.map(f => <FN key={f.id} f={f} font="font-shefa" size="text-[13.5pt] leading-snug" hl={sourceHighlight} rd={sourceReduce} />)}
            </div>
          </div>
          <div>
            <ColumnDivider label={storyColumnName} />
            <div className="space-y-2">
              {storyFootnotes.map(f => <FN key={f.id} f={f} font="font-frank" size="text-[7pt] leading-relaxed" hl={sourceHighlight} rd={sourceReduce} />)}
            </div>
          </div>
        </div>
      </div>
  ...
  const floatSize  = torahIsTaller ? 'text-[7pt] leading-relaxed' : 'text-[13.5pt] leading-snug'
  ...
  const wrapSize   = torahIsTaller ? 'text-[13.5pt] leading-snug' : 'text-[7pt] leading-relaxed'
